...
bool ProbeVisitor::VisitMemberExpr(MemberExpr *E) {
  ...
  string rhs = rewriter_.getRewrittenText(expansionRange(SourceRange(rhs_start, E->getMemberLoc())));
  string base_type = base->getType()->getPointeeType().getAsString();
  string pre, post;
  pre = "BPF_CORE_READ(";
  post = string(", ") + rhs + ")";
  rewriter_.InsertText(expansionLoc(GET_BEGINLOC(E)), pre);
  rewriter_.ReplaceText(expansionRange(SourceRange(member, E->getMemberLoc())), post);
  return true;
}
